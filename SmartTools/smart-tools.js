...
function initFilters(){let img=null;let names=Object.keys(filterPresets);$('#filterGrid').innerHTML=names.map(n=>`<button class="filter-btn ${n==='Original'?'active':''}" data-f="${n}"><span>${n==='Grayscale'?'◐':n==='Sepia'?'◒':n==='Blur'?'◌':'✦'}</span>${n}</button>`).join('');const vals={brightness:0,contrast:0,saturate:0,blur:0,sepia:0,hue:0};let chosen='Original';let render=()=>{if(!img)return;let p={...filterPresets[chosen],...vals};let c=$('#fCanvas'),maxW=760,maxH=500,sc=Math.min(maxW/img.width,maxH/img.height,1);c.width=Math.round(img.width*sc);c.height=Math.round(img.height*sc);let ctx=c.getContext('2d');ctx.filter=`brightness(${100+(p.brightness||0)}%) contrast(${100+(p.contrast||0)}%) saturate(${100+(p.saturate||0)}%) sepia(${p.sepia||0}%) hue-rotate(${p.hue||0}deg) blur(${p.blur||0}px)`;ctx.drawImage(img,0,0,c.width,c.height);$('#fInfo').textContent=`${img.width} × ${img.height}px • ${chosen}`};$('#fFile').onchange=async()=>{img=await fileToImg($('#fFile').files[0]);render()};$$('.filter-btn').forEach(b=>b.onclick=()=>{$$('.filter-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');chosen=b.dataset.f;Object.assign(vals,{brightness:0,contrast:0,saturate:0,blur:0,sepia:0,hue:0});Object.keys(vals).forEach(k=>{let el=$('#f'+({saturate:'s',brightness:'b',contrast:'c',blur:'bl',sepia:'sp',hue:'h'}[k]));if(el)el.value=0});updateLabels();render()});['b','c','s','bl','sp','h'].forEach(id=>$('#f'+id).oninput=()=>{let map={b:'brightness',c:'contrast',s:'saturate',bl:'blur',sp:'sepia',h:'hue'};vals[map[id]]=+$('#f'+id).value;updateLabels();render()});function updateLabels(){[['b',''],['c',''],['s',''],['bl',''],['sp',''],['h','°']].forEach(([x,u])=>$('#f'+x+'O').textContent=$('#f'+x).value+u)}$('#resetFilter').onclick=()=>{$$('.filter-btn').forEach(x=>x.classList.remove('active'));$$('.filter-btn')[0].classList.add('active');chosen='Original';Object.keys(vals).forEach(k=>vals[k]=0);['b','c','s','bl','sp','h'].forEach(x=>$('#f'+x).value=0);updateLabels();render()};$('#downloadFilter').onclick=()=>{if(!img)return alert('Choose an image first.');let c=$('#fCanvas');saveCanvas(c,'filtered-image.png');let b=$('#downloadFilter');b.textContent='✓ Downloaded';setTimeout(()=>b.textContent='⬇ Download PNG',1200)}}
